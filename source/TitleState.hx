...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		gfDance.frames = Paths.getSparrowAtlas('title/BF');
		gfDance.animation.addByPrefix('idle', 'BF idle dance instance 1', 24);
		...
		snas = new FlxSprite(-150, -100);
		snas.frames = Paths.getSparrowAtlas('title/SansCircle');
		snas.antialiasing = ClientPrefs.globalAntialiasing;
		snas.setGraphicSize(Std.int(snas.width * 1.6));
		snas.x -= 305;
		snas.y += 240;
		snas.updateHitbox(); 
		add(snas);
		
		cuphead = new FlxSprite(-150, -100);
		cuphead.frames = Paths.getSparrowAtlas('title/CupCircle');
		cuphead.antialiasing = ClientPrefs.globalAntialiasing;
		cuphead.setGraphicSize(Std.int(cuphead.width * 1.6));
		cuphead.x -= 250;
		cuphead.y += 220;
		cuphead.updateHitbox();
		add(cuphead);
		
		bendy = new FlxSprite(-150, -100);
		bendy.frames = Paths.getSparrowAtlas('title/BendyCircle');
		bendy.antialiasing = ClientPrefs.globalAntialiasing;
		bendy.setGraphicSize(Std.int(bendy.width * 1.6));
		bendy.x -= 100;
		bendy.y += 200;
		bendy.updateHitbox();
		add(bendy); 
		...
		titleText.animation.addByPrefix('idle', "Button instance 1", 24);
